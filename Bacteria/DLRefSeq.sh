...
time python3 Parallelize.py -n 50
...
python3 RenamePGDBs.py #Do not delete PGDBs duplicated rajout os remove
...
if [ $1 == Microcyc ]; then
	...
	python3 3%KeepOnlyTip.py ### comment probleme
	python3 DLMicrocycandExtractOnlyDownloads.py
	python3 DLMicrocycandExtractWithoutdownloads.py #### spliter en 2 script. Le deuxieme script est refait. ajout de try except
fi
python3 GenerateTaxonomyLineageAfterMicroCyc.py
python3 CalculateOldScore.py
python3 CountStrainPerSpecies.py
python3 PrepareTSVforDatabase.py #Changement
python3 getClassesOfPathwayremake.py #Sort la hierarchy de Metacyc pathway.dat & un CalculateOldScore
